...
public class TeamAdapter extends RecyclerView.Adapter<TeamAdapter.MyViewHolder> {
	...
	private String getTeamRank(String teamNumber, ArrayList<Rank> ranks) {
		if(ranks != null && ranks.size() > 0 && ranks.get(0).getRankings() != null) {
			for (int i = 0; i < ranks.get(0).getRankings().length; i++) {
				if (ranks.get(0).getRankings()[i].getTeam_key().equals(teamNumber)) return " Ranked #" +
						ranks.get(0).getRankings()[i].getRank();
			}
		}
		return "";
	}
